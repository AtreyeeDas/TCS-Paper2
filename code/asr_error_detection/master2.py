...
class MasterExperimentSuite:
    ...
    def stage_01_controlled_baseline(self):
        ...
        # ----------------------------------------------------------------------
        # SAVE DEPLOYMENT/REPRODUCIBILITY ARTIFACTS FOR CONTROLLED BASELINE
        # ----------------------------------------------------------------------
        
        # 1. Save the Model
        model_path = MODELS_CACHE_DIR / "controlled_detector_B.joblib"
        joblib.dump(det_b, model_path)
        
        # 2. Save Exact Feature Ordering
        controlled_feature_columns_path = MODELS_CACHE_DIR / "controlled_detector_B_features.json"
        with open(controlled_feature_columns_path, "w") as f:
            json.dump(df_fb.columns.tolist(), f, indent=2)
            
        # 3. Save Validation Threshold
        controlled_threshold_path = MODELS_CACHE_DIR / "controlled_detector_B_threshold.json"
        with open(controlled_threshold_path, "w") as f:
            json.dump({
                "threshold": float(thresh_b),
                "selection_method": "best_F1_on_validation_split"
            }, f, indent=2)
            
        # 4. Preserve existing result file
        with open(out_dir / "thresholds.json", "w") as f:
            json.dump({"detector_b": thresh_b}, f, indent=4)

        # Explicit Audit Prints
        self.log("MODEL-SAVE", f"Controlled Detector B saved to: {model_path}")
        self.log("MODEL-SAVE", f"Controlled Detector B feature schema saved to: {controlled_feature_columns_path}")
        self.log("MODEL-SAVE", f"Controlled Detector B threshold saved to: {controlled_threshold_path}")

        # Post-Save Verification
        try:
            loaded_model = joblib.load(model_path)
            with open(controlled_feature_columns_path, "r") as f:
                loaded_features = json.load(f)
            with open(controlled_threshold_path, "r") as f:
                loaded_thresh = json.load(f)["threshold"]
                
            if len(loaded_features) != len(df_fb.columns):
                raise ValueError("Feature JSON has incorrect length.")
            if loaded_features != df_fb.columns.tolist():
                raise ValueError("Feature JSON does not equal original feature list.")
            if abs(loaded_thresh - thresh_b) > 1e-9:
                raise ValueError("Threshold JSON value mismatch.")
                
            # Test model execution
            loaded_model.predict_proba(df_fb[val_mask][loaded_features].values)
            
            self.log("MODEL-AUDIT", "Controlled Detector B artifact verification: PASS")
        except Exception as e:
            self.log("MODEL-AUDIT", f"Controlled Detector B artifact verification: FAIL. Reason: {e}")
            raise RuntimeError(f"Controlled detector artifacts failed verification: {e}")
