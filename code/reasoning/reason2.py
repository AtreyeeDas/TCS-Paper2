        ...
        with open(ckpt_path, 'a') as ckpt_file:
            for i in tqdm(range(len(self.df)), desc="Deterministic & Fast Assignment"):
                ...
                for head in class_prototypes.keys():
                    ...
                    # FIX: Safely handle if an ontology head only has 1 defined class
                    if len(top_2_idx) >= 2:
                        ...
                        idx1 = idx2 = top_2_idx[0]
                        sim1 = sim2 = sims[idx1]
                        margin = 0.0
                        class1 = class2 = class_prototypes[head]["names"][idx1]
...
Return ONLY valid JSON: {{"selected_class": "EXACT_APPROVED_CLASS_NAME", "reason": "brief reason"}}"""
        ...
        for head in class_prototypes.keys():
            ...
            for cls_name, count in counts.items():
                ...
                freq_data.append({"head": head, "canonical_class": cls_name, "count": count, "percentage": round(count/total_valid*100, 2) if total_valid > 0 else 0})
    ...
    def stage_06_learnability_gate(self):
        ...
        for head, target in Config.LEARNABILITY_TARGETS.items():
            ...
            # FIX: Properly filter NaN values (blank rows from partial checkpointing) and cast to str
            valid_idx = df[col].notna() & ~df[col].isin(["OUTLIER_REVIEW", "MASK"])
            X = whisper_embs[valid_idx]
            y = df.loc[valid_idx, col].astype(str).values
