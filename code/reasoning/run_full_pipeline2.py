...
def main():
    ...
    # --------------------------------------------------------
    # ROBUST SAMPLE-ID PAIRING
    # --------------------------------------------------------
    master_unseen["sample_id"] = master_unseen["sample_id"].astype(str).str.strip()
    paired["sample_id"] = paired["sample_id"].astype(str).str.strip()

    paired_small = paired[available_pair_cols].drop_duplicates("sample_id")

    df = master_unseen.merge(
        paired_small, on="sample_id", how="left", indicator=True
    )

    # Keep every unseen sample. Missing saved error pairs are clean/negative
    # examples; they are never counted as strict induced errors.
    df["paired_transcript_source"] = np.where(
        df["_merge"] == "both", "saved_ASR_pair", "clean_fallback"
    )

    if "clean_whisper_transcript" in df.columns:
        clean_fallback = df["clean_whisper_transcript"].fillna(df["ground_truth"])
    else:
        clean_fallback = df["ground_truth"]

    df["corrupted_whisper_transcript"] = (
        df["corrupted_whisper_transcript"]
        .fillna(clean_fallback).fillna("").astype(str).str.strip()
    )
    df.drop(columns=["_merge"], inplace=True)

    print(f"Unseen samples: {len(df)}")
    print("Saved erroneous transcript pairs:", int((df["paired_transcript_source"] == "saved_ASR_pair").sum()))
    print("Clean fallback samples:", int((df["paired_transcript_source"] == "clean_fallback").sum()))
    ...
    for _, row in tqdm(
        ...
        has_saved_error_pair = int(
            row["paired_transcript_source"] == "saved_ASR_pair"
        )

        strict_error = int(
            has_saved_error_pair == 1
            and clean_text_correct == 1
...
""".strip()
        ...
        out = {
            ...
            "paired_transcript_source": row["paired_transcript_source"],
            "has_saved_error_pair": has_saved_error_pair,
