...
def run_diagnostic():
    ...
    # Safely handle 'split' column if missing
    ...
    for idx, row in tqdm(df_unseen.iterrows(), total=len(df_unseen), desc="Generating Fixed-Encoder Candidates"):
        ...
        # Audio robustness check: handles if sample_id already contains .wav
        audio_path = AUDIO_DIR / sample_id if str(sample_id).endswith('.wav') else AUDIO_DIR / f"{sample_id}.wav"
        
        if not audio_path.exists() or sample_id in processed_audio: 
            continue
            
        processed_audio.add(sample_id)
        
        # Safely extract target terms from likely column names
        target_term = ""
    ...
    # SAFETY CHECK: Prevent downstream crashes if all samples were skipped
    if len(all_candidates) == 0:
        print("\n" + "="*60)
        print("[!] CRITICAL ERROR: 0 candidates were generated.")
        print("    Every sample in the loop was skipped.")
        print("    Please check:")
        print("    1. Do the .wav files exist in your AUDIO_DIR?")
        print("    2. Is your target term column populated in the dataset CSV?")
        print("="*60)
        return
    ...
    audit_data["actual_runtime_feature_count"] = len(expected_features)  # Fixes the UnboundLocalError
