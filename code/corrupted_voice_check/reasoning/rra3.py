...
def load_cached_whisper_encoder(sample_id, device=DEVICE):
    ...
    for p in candidates:
        if p.exists():
            ...
            if tensor.dim() == 1 and tensor.shape[0] == 512:
                tensor = tensor.unsqueeze(0)  # [512] -> [1, 512]
            elif tensor.dim() == 2 and tensor.shape[-1] == 512:
                tensor = tensor.unsqueeze(0)  # [T, 512] -> [1, T, 512]
            elif tensor.dim() == 3 and tensor.shape[0] == 1 and tensor.shape[-1] == 512:
                pass  # [1, T, 512] -> unchanged
            else:
                raise ValueError(f"Invalid cached embedding shape {tensor.shape} for sample {sample_id}. Expected [512], [T, 512], or [1, T, 512].")
...
def run_reasoning_experiment():
    ...
    # Initialize Native Offline Metrics
    ...
    for idx, row in tqdm(df_samples.iterrows(), total=len(df_samples)):
        ...
        max_enc_diff = max(torch.max(torch.abs(enc_out_L0 - enc_out_L1)).item(), torch.max(torch.abs(enc_out_L0 - enc_out_L2)).item())
        assert max_enc_diff == 0.0, "Encoder representations altered!"

        # 2. VOICE-NLU (ONCE)
        if DEVICE == "cuda": torch.cuda.synchronize()
        t0_v = time.perf_counter()
        
        # DOWNSTREAM SHAPE FIX: Safely route based on encoder dimensions
        if enc_out.dim() == 3:
            emb_512 = enc_out.mean(dim=1).cpu().numpy().astype(np.float32)
        elif enc_out.dim() == 2:
            emb_512 = enc_out.cpu().numpy().astype(np.float32)
        else:
            raise ValueError(f"Unexpected encoder output shape before Voice-NLU: {enc_out.shape}")
            
        assert emb_512.shape == (1, 512), f"Expected Voice-NLU input shape (1, 512), got {emb_512.shape}"
        ...
        baseline_bs_f1 = {} # Store Baseline BS per level for correction computation
        ...
        if sanity_count < 10:
            ...
            print(f"Encoder Equality: torch.equal() PASSED. Max diff: {max_enc_diff}. Shape: {list(enc_out.shape)}")
