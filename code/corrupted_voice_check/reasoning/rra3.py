...
- Detector B receives exact extracted features using the authoritative feature_extractor.py.
...
def run_reasoning_experiment():
    ...
    for dom, terms in CANDIDATE_DISTRACTORS.items():
        ...
        # Bulletproof Fallback: if filtering empties the list, revert to the raw terms to prevent IndexError
    ...
    for idx, row in tqdm(df_samples.iterrows(), total=len(df_samples)):
        ...
        for level in [0, 1, 2]:
            ...
            # Fix: extract_sample_features accepts pd.Series exactly as authored in feature_extractor.py
