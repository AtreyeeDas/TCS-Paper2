        ...
        for level in [0, 1, 2]:
            ...
            for cond in ["Baseline", "Voice-NLU", "Detector-Gated"]:
                ...
                if cond == "Baseline":
                    ...
                    is_corr_succ = 1 if (level > 0 and diag.get(f"corruption_success_L{level}", 0) == 1 and bs_f1 > baseline_bs_f1[level] and dist_in == 0) else 0
                    is_false_corr = 1 if (level == 0 and bs_f1 < baseline_bs_f1[level]) else 0

                reasoning_evals.append({"sample_id": sample_id, "level": level, "cond": cond, "bs_f1": bs_f1, "rouge_l": rg_l, "bleu": bl_s, "dist_adopted": dist_in, "corr_success": is_corr_succ, "false_corr": is_false_corr})
