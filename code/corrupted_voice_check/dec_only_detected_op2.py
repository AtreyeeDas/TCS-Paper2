...
# Secondary dataset strictly for vocabulary building
NLU_DATASET_CSV = ROOT_DIR / "dataset" / "nlu_robust_6000_scenario_paraphrase.csv"
...
def build_dataset_vocabulary(dfs):
    ...
    for df in dfs:
        ...
        # Added semantic label columns to exclusion search
        text_cols = [c for c in df.columns if any(k in c.lower() for k in ['transcript', 'target', 'term', 'domain', 'topic', 'subdomain', 'document', 'label'])]
...
def run_diagnostic():
    ...
    df_nlu_raw = pd.read_csv(NLU_DATASET_CSV) if NLU_DATASET_CSV.exists() else None
    
    # 2. Build Dataset Vocabulary & Distractor Filter (across all textual/semantic columns)
    dataset_vocab = build_dataset_vocabulary([df_eval_raw, df_nlu_raw])
    valid_distractors = {}
    total_candidates = 0
    excluded_count = 0
    ...
    assert len(df_samples) > 0, "FATAL: 0 audio files found! Verify AUDIO_DIR mapping."
    ...
    for idx, row in tqdm(df_samples.iterrows(), total=len(df_samples), desc="Generating Fixed-Encoder Candidates"):
        ...
        if sample_id in processed_audio: continue
        processed_audio.add(sample_id)
        
        # Parse target terms strictly
        target_term = ""
        if 'target_terms' in row and pd.notna(row['target_terms']):
            raw_terms = str(row['target_terms']).lower()
            # If multiple terms exist separated by semicolon, grab the first primary term
            target_term = raw_terms.split(';')[0].strip()
            
        if not target_term or target_term == 'nan': 
            continue
    ...
    if len(all_candidates) == 0:
        print("\n" + "="*60)
        print("[!] CRITICAL ERROR: 0 candidates were generated.")
        print("    Every sample in the loop was skipped.")
        print("="*60)
        return
