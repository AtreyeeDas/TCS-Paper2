...
class Config:
    ...
    MIN_CLASS_SIZE = 50  # <-- ADDED THIS MISSING CONFIG VARIABLE
...
Return ONLY valid JSON: {{"selected_class": "EXACT_APPROVED_CLASS_NAME", "reason": "brief reason"}}"""
    ...
    def stage_06_learnability_gate(self):
        ...
        # Safely squeeze and mean pool sequence length
        whisper_embs = np.squeeze(whisper_embs)
        if whisper_embs.ndim == 3:
            whisper_embs = whisper_embs.mean(axis=1)
        elif whisper_embs.ndim > 2:
        ...
        for head, target in Config.LEARNABILITY_TARGETS.items():
            ...
            # Ignore NaNs and missing classes using string casting safely
