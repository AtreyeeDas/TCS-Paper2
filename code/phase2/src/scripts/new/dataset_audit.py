...
def resolve_audio_path(rel_path: str, audio_lookup: dict) -> str:
    if pd.isna(rel_path) or str(rel_path).strip() == "": return None
    str_path = str(rel_path).strip()
    if os.path.isabs(str_path) and os.path.exists(str_path): return os.path.abspath(str_path)
    direct_candidate = os.path.abspath(os.path.join(Config.DATA_DIR, str_path))
    if os.path.exists(direct_candidate): return direct_candidate
...
def audit_and_split():
    ...
    # Detect synthetic samples
    valid_df["is_synthetic"] = valid_df["resolved_path"].str.lower().str.contains("synthetic")
    synthetic_count = valid_df["is_synthetic"].sum()
    print(f"[+] Found {synthetic_count} synthetic samples. Forcing to TRAIN ONLY.")

    # Grouping by transcript
    valid_df["group_id"] = valid_df["transcript"].fillna(valid_df["audio_path"]).astype(str).str.lower().str.strip()
    ...
    for head in Config.HEADS:
        unique_labels = sorted([l for l in train_df[head].unique() if str(l) != Config.MASK_TOKEN])
    ...
    for head in Config.HEADS:
        for cls in unique_labels:
            ...
            distribution_records.append({
                "head": head, "class": cls, "train": tr_c, "val": vl_c, "test": te_c, "total": tr_c+vl_c+te_c
            })
    pd.DataFrame(distribution_records).to_csv(os.path.join(splits_dir, "class_distribution_by_split.csv"), index=False)

    print("[+] Dataset audit and split isolation completed successfully.")
