...
class IntrospectionEngine:
    def __init__(self, config: WhisperConfig):
        ...
        # We load strictly from local path, forcing memory efficient attention via native PyTorch SDPA
        self.processor = AutoProcessor.from_pretrained(config.model_path, local_files_only=True)
        self.model = WhisperForConditionalGeneration.from_pretrained(
            config.model_path,
            torch_dtype=config.dtype,
            local_files_only=True,
            attn_implementation="sdpa" # Changed from flash_attention_2 to bypass aarch64 compile issues
        ).to(config.device)
        self.model.eval()
    ...
    def process(self, audio_array: np.ndarray):
        ...
        # Force specific tokens for language and task
